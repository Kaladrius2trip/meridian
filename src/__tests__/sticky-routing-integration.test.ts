...
describe("Integration: sticky profile routing (#383)", () => {
  ...
  // Fork divergence from upstream #383: session-to-profile affinity is always
  // on here (least-loaded distribution), so sessions spread across profiles
  // even without MERIDIAN_ROUTING instead of piling onto the first profile.
  it("GOLDEN: without MERIDIAN_ROUTING sessions distribute across profiles and stay sticky (fork affinity)", async () => {
